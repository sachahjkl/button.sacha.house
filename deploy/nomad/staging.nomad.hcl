variable "image" {
  ...
  validation {
    condition     = strlen(var.image) == 108 && substr(var.image, 0, 44) == "ghcr.io/sachahjkl/button.sacha.house@sha256:"
    error_message = "The image must use the Button GHCR repository and an exact SHA-256 digest."
  }
}
